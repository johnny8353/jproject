...
function fPopCalendar(popCtrl, dateCtrl,strDate)
{
	...
	if(c.length < 1)
	{
		...
		divHtml = divHtml + "&nbsp;<SELECT name='tbSelYear'onChange='fUpdateCal(tbSelYear.value, tbSelMonth.value)' Victor='Won'>";
		for(i=1900;i<=2050;i++)
		{
				divHtml = divHtml + "<OPTION value='"+i+"'>"+i+" 年</OPTION>";
		}
		divHtml = divHtml + "</SELECT>";
		divHtml = divHtml + "&nbsp;<select name='tbSelMonth' onChange='fUpdateCal(tbSelYear.value, tbSelMonth.value)' Victor='Won'>";
		for (i=0; i<12; i++)
		{
