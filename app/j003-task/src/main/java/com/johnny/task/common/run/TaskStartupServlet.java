...
public class TaskStartupServlet extends HttpServlet {
	...
	public void inithread() throws ServletException {
		...
		// 加载所有需要的调度线程
		for (int i = 0; i < threadVOs.size(); i++) {
			TaskVO taskVO = threadVOs.get(i);
			TaskBO taskBO = new TaskBO();
			log.debug(taskVO.toString());
			try {
//				Class threadClass =Class.forName(taskVO.getClassName());
//				BaseTask bst = (BaseTask) threadClass.newInstance();
			...
			log.info(this.getClass().getName() + taskVO.toString());
		}
	}
	...
	protected Object getBean2(Class c) {
		WebApplicationContext wac = ContextLoader
				.getCurrentWebApplicationContext();
